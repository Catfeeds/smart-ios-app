...
@interface HTMajorSchoolCell () <UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>
...
@implementation HTMajorSchoolCell

static NSString *kHTMajorDetailSchoolCollectionCellIdentifier = @"kHTMajorDetailSchoolCollectionCellIdentifier";
...
- (void)setModel:(NSArray <HTMajorSchoolModel *> *)model row:(NSInteger)row {
	_modelArray = model;
	NSMutableArray *modelSizeArray = [@[] mutableCopy];
	HTMajorCountryCollectionCell *cell = [[HTMajorCountryCollectionCell alloc] init];
	self.collectionView.frame = CGRectMake(0, 0, HTSCREENWIDTH - 30, 0);
	[model enumerateObjectsUsingBlock:^(HTMajorSchoolModel *model, NSUInteger index, BOOL * _Nonnull stop) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
	UICollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:kHTMajorDetailSchoolCollectionCellIdentifier forIndexPath:indexPath];
...
- (UICollectionView *)collectionView {
	if (!_collectionView) {
		...
		[_collectionView registerClass:[HTMajorCountryCollectionCell class] forCellWithReuseIdentifier:kHTMajorDetailSchoolCollectionCellIdentifier];
	}
