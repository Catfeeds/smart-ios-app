...
@interface HTAnswerSingleController ()
...
@implementation HTAnswerSingleController
...
- (void)initializeDataSource {
	...
	[self.tableView ht_setRefreshBlock:^(NSString *pageSize, NSString *currentPage) {
		...
		[HTRequestManager requestAnswerListWithNetworkModel:networkModel answerTagString:weakSelf.answerTagModel.ID pageSize:pageSize currentPage:currentPage complete:^(id response, HTError *errorModel) {
			...
			[modelArray enumerateObjectsUsingBlock:^(HTAnswerModel *answerModel, NSUInteger idx, BOOL * _Nonnull stop) {
				answerModel.tags = @[];
			}];
...
- (void)initializeUserInterface {
	self.navigationItem.title = self.answerTagModel.name;
