...
- (PHImageRequestID)getPhotoWithAsset:(id)asset photoWidth:(CGFloat)photoWidth completion:(void (^)(UIImage *photo,NSDictionary *info,BOOL isDegraded))completion progressHandler:(void (^)(double progress, NSError *error, BOOL *stop, NSDictionary *info))progressHandler networkAccessAllowed:(BOOL)networkAccessAllowed {
    if ([asset isKindOfClass:[PHAsset class]]) {
        ...
        NSArray *array = @[[asset valueForKey:@"filename"] ? [asset valueForKey:@"filename"] : @""];
        NSString *string = @".gif";
        ...
        if ([array filteredArrayUsingPredicate:predicate].count) {
            NSDictionary *assetGetBundleDictionary = objc_getAssociatedObject(asset, _cmd);
            if (assetGetBundleDictionary.count) {
                UIImage *result = assetGetBundleDictionary[@"result"];
                NSDictionary *info = assetGetBundleDictionary[@"info"];
                if (completion && result && info) completion(result, info, [[info valueForKey:PHImageResultIsDegradedKey] boolValue]);
            } else {
                imageRequestID = [[PHImageManager defaultManager] requestImageDataForAsset:asset options:option resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
                    if (imageData) {
                        UIImage *result = [YYImage imageWithData:imageData];
                        result = [self fixOrientation:result];
                        if (completion) completion(result, info, [[info objectForKey:PHImageResultIsDegradedKey] boolValue]);
                        if (result && info) {
                            NSDictionary *assetSetBundleDictionary = @{@"result":result, @"info":info};
                            objc_setAssociatedObject(asset, _cmd, assetSetBundleDictionary, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
                        }
                    }
                }];
            }
        } else {
            imageRequestID = [[PHImageManager defaultManager] requestImageForAsset:asset targetSize:imageSize contentMode:PHImageContentModeAspectFit options:option resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
