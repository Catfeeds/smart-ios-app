...
@property (nonatomic, copy) NSArray *originModelArray;

@property (nonatomic, copy) void(^dragedModelArrayBlockName)(NSArray *dragedModelArray);
...
- (void)updateDataSource{
	...
	} else if (self.originModelArray) {
		[tempArray addObjectsFromArray:self.originModelArray];
	}
    //判断原始数据源是否为嵌套数组
	...
	} else if (self.dragedModelArrayBlockName) {
		self.dragedModelArrayBlockName(tempArray);
		self.originModelArray = tempArray;
	}
}
...
- (void)setOriginModelArray:(NSArray *)modelArray {
	_originModelArray = modelArray;
}

- (void)setDragedModelArrayBlock:(void(^)(NSArray *modelArray))dragedModelArrayBlock {
	self.dragedModelArrayBlockName = dragedModelArrayBlock;
}
